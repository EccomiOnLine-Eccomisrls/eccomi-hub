...
async def _require_ceo(authorization: str | None) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=12.0) as client:
        auth_response = await client.get(f"{SUPABASE_URL}/auth/v1/user", headers={"apikey": SUPABASE_ANON_KEY, "Authorization": f"Bearer {token}"})
        if auth_response.status_code != 200:
            raise HTTPException(status_code=401, detail="Sessione scaduta o non valida.")
        user = auth_response.json()
        user_id = str(user.get("id") or "")
        profile_response = await client.get(f"{SUPABASE_URL}/rest/v1/hub_profiles", headers=_service_headers(), params={"user_id": f"eq.{user_id}", "role": "eq.ceo", "active": "eq.true", "select": "user_id,email,role", "limit": "1"})
...
async def _find_or_create_auth_user(email: str) -> str:
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        ...
        payload = response.json(); users = payload.get("users", payload if isinstance(payload, list) else [])
        for user in users:
...
async def _list_responsibles() -> list[ResponsibleProfile]:
    async with httpx.AsyncClient(timeout=12.0) as client:
        response = await client.get(f"{SUPABASE_URL}/rest/v1/hub_profiles", headers=_service_headers(), params={"role":"eq.manager","select":"user_id,ec_id,email,full_name,role,ecosystem_keys,active","order":"created_at.asc"})
    if response.status_code != 200:
...
async def list_responsibles(authorization: str | None = Header(default=None)) -> list[ResponsibleProfile]:
    await _require_ceo(authorization); return await _list_responsibles()
...
async def create_responsible(body: ResponsibleInput, authorization: str | None = Header(default=None)) -> ResponsibleProfile:
    ...
    email = str(body.email).strip().lower(); full_name = body.full_name.strip(); ecosystem_key = body.ecosystem_key.strip().lower()
    user_id = await _find_or_create_auth_user(email)
    if not user_id: raise HTTPException(status_code=502, detail="Identità HUB non disponibile.")
    current = await _list_responsibles(); existing = next((item for item in current if item.user_id == user_id), None)
    ...
    async with httpx.AsyncClient(timeout=12.0) as client:
        response = await client.post(f"{SUPABASE_URL}/rest/v1/hub_profiles", headers={**_service_headers(),"Prefer":"resolution=merge-duplicates,return=representation"}, params={"on_conflict":"user_id"}, json=row)
    if response.status_code not in (200,201):
        if response.status_code in (401,403): raise HTTPException(status_code=503, detail="La Service Role non può ancora scrivere in hub_profiles. Concedi INSERT e UPDATE alla tabella e riprova.")
        raise HTTPException(status_code=502, detail=f"Salvataggio responsabile non riuscito: {response.text[:300]}")
    payload=response.json(); saved=payload[0] if isinstance(payload,list) and payload else row
...
async def set_responsible_status(user_id: str, body: ResponsibleStatusInput, authorization: str | None = Header(default=None)) -> ResponsibleProfile:
    await _require_ceo(authorization)
    async with httpx.AsyncClient(timeout=12.0) as client:
        response = await client.patch(f"{SUPABASE_URL}/rest/v1/hub_profiles", headers={**_service_headers(),"Prefer":"return=representation"}, params={"user_id":f"eq.{user_id}","role":"eq.manager"}, json={"active":body.active})
    if response.status_code not in (200,204): raise HTTPException(status_code=502, detail="Aggiornamento stato responsabile non riuscito.")
    payload=response.json() if response.content else []
    if not payload: raise HTTPException(status_code=404, detail="Responsabile non trovato.")
...
def _patched_fastapi_init(self: FastAPI, *args: Any, **kwargs: Any) -> None:
    _original_fastapi_init(self, *args, **kwargs); _install_routes(self)
